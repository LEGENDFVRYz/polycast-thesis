"""
background.pipeline_ekf.tracker
===============================
...
It intentionally contains no matplotlib/UI code.

This is the application-side entry point for the *EKF* pipeline.  The other
...
# Import from this package's own config, which is the copy of the kuru config
# main_ekf.py uses.  The original try/except fallback silently used zero UWB
# offsets and zero tip-offset when the root config.py lacked these keys, which
# caused systematic trilateration errors (up to 19 cm per anchor) and no
# lever-arm correction on the pen tip -- so these imports stay unguarded and
# fail loudly instead.
from background.pipeline_ekf.preprocessor import UWBPreprocessor, IMUPreprocessor
from background.pipeline_ekf.ekf_fusion import AsyncEKFFusionEngine
from background.pipeline_ekf.config import UWB_OFFSETS, TIP_OFFSET_FROM_TAG_M
