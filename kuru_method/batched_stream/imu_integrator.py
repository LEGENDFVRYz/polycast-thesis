...
-------------------
    ...
    LINEAR_ACCEL     → gravity-compensated acceleration in body frame (m/s²)
...
def quat_to_rotmat(qx: float, qy: float, qz: float, qw: float) -> np.ndarray:
    """
    Unit quaternion → 3×3 rotation matrix R  where  v_world = R @ v_body.
...
class IMUIntegrator:
    ...
    -----
        ...
        for sample in imu_batch:
            a_wb = integrator.get_wb_acceleration(sample['quat'], sample['acc'])
            # a_wb → np.ndarray([ax_wb, ay_wb]) in m/s²

    Tuning guide
    ------------
    HEADING_INIT_SAMPLES (default 30):
        Samples averaged before locking the board heading.
        30 samples @ 100 Hz ≈ 300 ms.  Increase to 60 if the first
        packets have erratic marker orientation.
        Call reset_heading() to re-estimate (e.g. after a board move).

    ACC_DEADBAND_MS2 (default 0.02 m/s²):
    ...
    ACC_CLAMP_MS2 (default 20.0 m/s²):
        Hard clamp per sample.  Rejects sensor glitches.
        Never set below the maximum expected stroke acceleration (~8 m/s²).
    """
    ...
    def get_wb_acceleration(self, q, a_body_raw):
        ...
        a_body_raw : array-like (3,) [ax, ay, az]  body-frame linear accel (m/s²)

        Returns
        -------
        a_wb : np.ndarray (2,) [a_wb_x, a_wb_y]  whiteboard 2D accel (m/s²)
        """
