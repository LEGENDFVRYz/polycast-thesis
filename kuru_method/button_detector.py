"""
button_detector.py  —  PolyCast Tactile Switch Contact Detector
================================================================
Hardware design (alternative to the spring-force sensor):
...
Firmware encoding (from imu_module.cpp — button variant):
...
What this detector does:
    ...
    2. Applies temporal debounce — suppresses contact-bounce glitches
       that occur in the first ~1–3 ms after the button makes/breaks contact
    3. Outputs a clean 0/1 binary state exactly like force_detector.py,
       so it is drop-in compatible with the FusionEngine

No EMA smoothing is needed here — the signal is already binary from
the firmware.  Smoothing it would only introduce lag.
"""
...
class ButtonContactDetector:
    """
    Pipeline for the tactile button alternative:
...
class ButtonDetectorDashboard:
    ...
    def _build_figure(self):
        ...
        self.fig.canvas.manager.set_window_title(
            'PolyCast — Tactile Button Contact Detector')
    ...
    def _ingest_packet(self, packet):
        for sample in packet['imu']:
            state, raw_state = self.detector.process(sample['force'])
...
def main():
    ...
    dashboard = ButtonDetectorDashboard(stream)
    mode_str  = 'CSV Playback' if stream.mode == 'csv' else 'Live Serial'
    print(f'🚀 Button Detector running in {mode_str} mode')
